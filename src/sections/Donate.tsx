...
function DetailRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-white/10 py-4 last:border-0">
      <div>
        <div className="text-xs font-semibold uppercase tracking-[0.15em] text-navy-200">{label}</div>
        <div className="mt-1 font-mono text-base font-semibold text-white sm:text-lg">{value}</div>
      </div>
      <button
        onClick={copy}
        aria-label={`Copy ${label}`}
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/20 text-navy-200 transition-colors hover:border-gold-400 hover:text-gold-400"
      >
      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        ...
        <div className="mt-16 grid items-stretch gap-8 lg:grid-cols-5">
          ...
          <Reveal delay={0.15} className="lg:col-span-3">
            <div className="h-full rounded-3xl bg-navy-900 p-9 shadow-2xl shadow-navy-900/25">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gold-500/15 text-gold-400">
                  <Landmark className="h-6 w-6" />
                </div>
                <div>
                  <h3 className="font-display text-2xl font-bold text-white">Bank Account Details</h3>
                  <p className="text-sm text-navy-200">Direct transfer / NEFT / RTGS / IMPS / SWIFT</p>
                </div>
              </div>
              <div className="mt-6">
                {bankDetails.map((d) => (
